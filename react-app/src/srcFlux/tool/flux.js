...
// 6.怎么去把flux中的数据进行改变，封装对外的接口（方法），想要数据改变时，执行这个接口，传入想要执行逻辑的action类型
//   供视图层执行的方法
// 7.视图中通过flux提供的接口传入类型，然后在接口中将类型和当前的state传入逻辑处理层，进行一系列的逻辑处理，形成新的state赋给当前的this.state
// 8. 此时数据库更新完毕，通知视图进行数据的重新获取，采用观察者模式实现视图对数据的监听，当数据库改变时（当dispatch执行时，触发监听事件），将所有视图中监听数据的监听函数都执行一遍，在监听函数中，进行数据的重新获取
// 9.此时需要监听对外接口，所以封装了要给observer的监听函数
